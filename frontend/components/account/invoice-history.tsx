...
export default function InvoiceHistory({
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      try {
        ...
        if (subscriptionId) {
          url = `/api/invoices/subscription/${encodeURIComponent(subscriptionId)}?limit=${limit}`;
        } else if (userId) {
          url = `/api/invoices?limit=${limit}`;
        }

        const response = await fetchBackend(url);
        const data = await parseBackendResponse<{ data: Invoice[] }>(response);
  ...
  const handleViewInvoice = async (invoiceId: string) => {
    try {
      const response = await fetchBackend(`/api/invoices/${encodeURIComponent(invoiceId)}`);
      const data = await parseBackendResponse<{ data: Invoice }>(response);
